  ...
  "node_modules/tinycolor2/tinycolor.js"(exports2, module2) {
    (function(Math2) {
      ...
      function hsvToRgb(h, s, v) {
        ...
        s = bound01(s, 100);
        v = bound01(v, 100);
        var i = Math2.floor(h), f2 = h - i, p = v * (1 - s), q = v * (1 - f2 * s), t = v * (1 - (1 - f2) * s), mod = i % 6, r = [v, q, p, p, t, v][mod], g = [t, v, v, q, p, p][mod], b = [p, p, t, v, v, q][mod];
        return { r: r * 255, g: g * 255, b: b * 255 };
      }
...
// node_modules/@dreamcomposer/pre/lib/MouseEventHandler.js
var MouseEventHandler = class {
  ...
  handlePointerEvent(event) {
    if (event.payload.type === "up") {
      ...
      this.dcController.container.dispatchEvent(event2);
    }
  ...
  sendEvent(type, event) {
    this.dcController.postMessage({
      ...
      payload: {
        type,
        clientX: event.clientX - this.dcController.container.getBoundingClientRect().left,
        clientY: event.clientY - this.dcController.container.getBoundingClientRect().top
      }
    });
  }
};

// node_modules/@dreamcomposer/pre/lib/DcController.js
var CONTAINER_INNER_HTML = `
...
var DcController = class {
   ...
   */
  async createIFrame() {
    this.iFrameContainer.innerHTML = '<iframe frameborder="0" title="Dreamcomposer" allowfullscreen style="width: 100%; height: 100%; color-scheme: auto;"></iframe>';
  ...
  addEventListener() {
    window.addEventListener("message", (event) => {
      var _a;
      if (!this.iframe || event.origin !== this.baseUrl.embed || event.source !== ((_a = this.iframe) === null || _a === void 0 ? void 0 : _a.contentWindow)) {
        return;
      }
      ...
      return new Promise((resolve, reject2) => {
        this.waitingForAck.set(nonce, (ackMessage) => {
          if (ackMessage.payload.error) {
            reject2(ackMessage.payload.error);
          } else {
            resolve();
          }
        });
...
// node_modules/@dreamcomposer/pre/lib/methods/access-token.js
...
// node_modules/@dreamcomposer/pre/lib/util/parseColorHex.js
function parseColorHex(color2) {
  const regex = /^#[0-9a-fA-F]{8}$|^#[0-9a-fA-F]{6}$|^#[0-9a-fA-F]{4}$|^#[0-9a-fA-F]{3}$/gm;
  if (!regex.test(color2)) {
    return false;
  }
  if (color2.length === 4 || color2.length === 5) {
    color2 = color2.split("").map((c) => c + c).join("").slice(1);
  }
  if (color2.length === 7) {
    color2 = color2 + "FF";
  }
...
// node_modules/@dreamcomposer/pre/lib/methods/background.js
function setBackgroundColor(dcController, colorHex) {
...
function setBackgroundImage(dcController, texture, options) {
  return dcController.postMessage({
...
function removeBackgroundImage(dcController) {
  return dcController.postMessage({
    channel: "set-background-image",
    payload: {
      texture: null
...
// node_modules/@dreamcomposer/pre/lib/methods/base-url.js
var _baseUrl = "https://dreamcomposer.de";
function setBaseUrl(baseUrl) {
  _baseUrl = baseUrl;
  if (typeof _baseUrl === "string" && _baseUrl.endsWith("/")) {
    _baseUrl = _baseUrl.substring(0, _baseUrl.length - 1);
  } else if (typeof _baseUrl === "object") {
    if (_baseUrl.pcm.endsWith("/")) {
      _baseUrl.pcm = _baseUrl.pcm.substring(0, _baseUrl.pcm.length - 1);
    }
    if (_baseUrl.embed.endsWith("/")) {
      _baseUrl.embed = _baseUrl.embed.substring(0, _baseUrl.embed.length - 1);
    }
  }
}
function getBaseUrl() {
...
function getPcmBaseUrl() {
  return typeof _baseUrl === "string" ? _baseUrl : _baseUrl.pcm;
}
function getEmbedBaseUrl() {
  return typeof _baseUrl === "string" ? _baseUrl : _baseUrl.embed;
}

// node_modules/@dreamcomposer/pre/lib/methods/cutout-texture.js
async function setCutoutTexture(dcController, data) {
  return dcController.postMessage({
    channel: "set-cutout-texture",
    payload: data !== null && data !== void 0 ? data : ""
  });
}

// node_modules/@dreamcomposer/pre/lib/methods/download-screenshot.js
function downloadScreenshot(dcController, options) {
...
// node_modules/@dreamcomposer/pre/lib/methods/effect-settings.js
var DEFAULT_EFFECT_SETTINGS = Object.freeze({
  fieldOfView: 0.75,
  bloomEnabled: true,
  bloomThreshold: 0.9,
  bloomWeight: 0.05,
  ...
  grainEnabled: false,
  grainIntensity: 30,
  imageProcessingBrightness: 1,
  imageProcessingContrast: 1,
...
function setEffectSettings(dcController, settings) {
  return dcController.postMessage({
    channel: "set-effects-settings",
    payload: {
...
// node_modules/@dreamcomposer/pre/lib/util/promise-queue.js
var waitingQueue = /* @__PURE__ */ new Map();
async function putInQueue(key, promise) {
  if (!waitingQueue.has(key)) {
    waitingQueue.set(key, []);
  }
  const list = waitingQueue.get(key);
  const p = new Promise((resolve) => {
    Promise.all([...list]).then(promise).then(() => {
      resolve();
...
// node_modules/@dreamcomposer/pre/lib/util/ensure-call-order.js
function ensureCallOrder(method2) {
  const id = Math.random().toString(36).substring(2);
  const t = (dcController, ...args) => {
    return putInQueue(`${dcController.id}|${id}`, async () => {
      return method2(dcController, ...args);
    });
  };
  return t;
}

// node_modules/@dreamcomposer/pre/lib/methods/embossing-height-texture.js
var setEmbossingHeightTexture = ensureCallOrder(_setEmbossingHeightTexture);
async function _setEmbossingHeightTexture(dcController, data) {
  let blob;
...
// node_modules/@dreamcomposer/pre/lib/pcm/item.js
var PermissionRole;
(function(PermissionRole2) {
  PermissionRole2["NONE"] = "NONE";
  PermissionRole2["USER"] = "USER";
  PermissionRole2["OWNER"] = "OWNER";
})(PermissionRole || (PermissionRole = {}));
function isPcmItem(obj) {
  return typeof obj === "object" && obj !== null && "id" in obj && "displayName" in obj;
}
var mapNativePcmItemToPcmItem = (item) => ({
  id: item.id,
  ...
  category: {
    ...
    description: item.category.description || void 0,
...
// node_modules/@dreamcomposer/pre/lib/pcm/util.js
var doNotSendAccessTokenToPcm = false;
function disablePcmAccessTokenCheck() {
  doNotSendAccessTokenToPcm = true;
}
var fetchPcmData = (url, accessToken) => fetch(url, {
  headers: {
    ...doNotSendAccessTokenToPcm ? {} : { "dc-application-access-token": accessToken }
  }
});
function checkAccessToken(accessToken) {
  if (accessToken === void 0) {
    throw new Error("You have to provide an access token either by setting a default one with dc.setAccessToken(...) or by providing it directly.");
  }
}
var itemCache = /* @__PURE__ */ new Map();
async function _getPcmItems(type, accessToken, baseUrl) {
...
function _invalidateCache(type, uuid, accessToken, baseUrl) {
  const keyMultiple = accessToken + "|" + type + "|" + baseUrl;
  if (itemCache.has(keyMultiple)) {
...
async function _getPcmItem(type, id, accessToken, baseUrl) {
  checkAccessToken(accessToken);
  const uuid = _removeReadablePrefixFromUUID(id);
  const keyMultiple = accessToken + "|" + type + "|" + baseUrl;
  if (itemCache.has(keyMultiple)) {
    const items = await itemCache.get(keyMultiple);
    const item2 = items.find((i) => i.id === uuid || i.alias === uuid);
    if (item2) {
      return item2;
    }
  }
...
// node_modules/@dreamcomposer/pre/lib/pcm/environment.js
function isPcmEnvironment(obj) {
  return isPcmItem(obj) && "alias" in obj && "url" in obj && "defaultRotation" in obj && "exposure" in obj;
}
async function getEnvironments(accessToken = getAccessToken(), baseUrl = getPcmBaseUrl()) {
  const items = await _getPcmItems("environments", accessToken, baseUrl);
  return items.map(mapNativePcmEnvironmentToLibrary);
}
async function getEnvironmentById(id, accessToken = getAccessToken(), baseUrl = getPcmBaseUrl()) {
  const item = await _getPcmItem("environments", id, accessToken, baseUrl);
  return item ? mapNativePcmEnvironmentToLibrary(item) : void 0;
}
var mapNativePcmEnvironmentToLibrary = (environment) => ({
...
// node_modules/@dreamcomposer/pre/lib/methods/environment.js
var setEnvironment = ensureCallOrder(_setEnvironment);
async function _setEnvironment(dcController, environment, options = {}) {
  var _a, _b, _c, _d;
  const payload = {};
  ...
  if (isPcmEnvironment(environment)) {
    ...
    payload.defaultRotation = (_b = options.defaultRotation) !== null && _b !== void 0 ? _b : environment.defaultRotation;
...
function setEnvironmentVisibility(dcController, visible) {
  return dcController.postMessage({
...
// node_modules/@dreamcomposer/pre/lib/pcm/geometry.js
function isPcmGeometry(obj) {
  return isPcmItem(obj) && "url" in obj && "defaultScale" in obj && "stages" in obj;
}
async function getGeometries(accessToken = getAccessToken(), baseUrl = getPcmBaseUrl()) {
  const items = await _getPcmItems("geometries", accessToken, baseUrl);
  return items.map(mapNativePcmGeometryToPcmGeometry);
}
async function getGeometryById(id, accessToken = getAccessToken(), baseUrl = getPcmBaseUrl()) {
  const item = await _getPcmItem("geometries", id, accessToken, baseUrl);
  return item ? mapNativePcmGeometryToPcmGeometry(item) : void 0;
}
async function createGeometry(temporaryFileId, config, accessToken = getAccessToken(), baseUrl = getPcmBaseUrl()) {
  checkAccessToken(accessToken);
  const pcmConfig = mapEditablePcmGeometryToNativeEditablePcmGeometry(config);
  const formData = { ...pcmConfig, temporaryFileId };
  ...
  _invalidateCache("geometries", void 0, accessToken, baseUrl);
  return response.json();
}
async function patchGeometry(geometryId, config, accessToken = getAccessToken(), baseUrl = getPcmBaseUrl()) {
  checkAccessToken(accessToken);
...
async function deleteGeometry(geometry, accessToken = getAccessToken(), baseUrl = getPcmBaseUrl()) {
  checkAccessToken(accessToken);
  try {
...
var mapEditablePcmGeometryToNativeEditablePcmGeometry = (data) => {
  var _a;
  return {
...
// node_modules/@dreamcomposer/pre/lib/methods/geometry.js
async function setGeometry(dcController, dataOrUrlOrIdOrPcm, options) {
  __setGeometry(dcController, dataOrUrlOrIdOrPcm, options);
}
var __setGeometry = ensureCallOrder(_setGeometry);
async function _setGeometry(dcController, dataOrUrlOrIdOrPcm, options) {
  var _a, _b, _c;
  const stages = {};
  ...
  if (isPcmGeometry(dataOrUrlOrIdOrPcm)) {
    const preset = options != null && "preset" in options ? options["preset"] : void 0;
  ...
  if (isPcmGeometry(dataOrUrlOrIdOrPcm)) {
    ...
    if (!dataOrUrlOrIdOrPcm.startsWith("http")) {
      ...
      if (!geometry) {
        throw new Error(`Cannot find geometry with id ${dataOrUrlOrIdOrPcm}. If you wanted to load a geometry from a URL, please also provide the protocol (i.e., "http(s)://").`);
      }
...
function convertGeometryPresetToGeometryOverrides(preset, geometry) {
  ...
  if (overrideConfig.presets) {
    ...
    if (presetData) {
      ...
      presetData.components.forEach((component) => {
        const property2 = overrideConfig.propertyOverrides.find((prop) => prop.id === component.property);
        if (property2) {
          const option = property2.options.find((option2) => option2.id === component.selection);
          if (option) {
            overrides.push({ target: property2.target, data: option.data });
          }
        }
      });
    }
  ...
  if (createResetOverrides) {
    overrides = overrideConfig.propertyOverrides.map((property2) => ({
      target: property2.target,
      data: []
    }));
  }
...
// node_modules/@dreamcomposer/pre/lib/methods/grid-visibility.js
function setGridVisibility(dcController, visible) {
  return dcController.postMessage({
    channel: "set-grid-visible",
    payload: visible
  });
}

// node_modules/@dreamcomposer/pre/lib/methods/init.js
function init(idOrElement, options) {
  var _a, _b;
  const container = idOrElement instanceof HTMLElement ? idOrElement : document.getElementById(idOrElement);
  ...
  if (accessToken === void 0) {
    throw new Error("You have to provide an access token either by setting a default one with dc.setAccessToken(...) or by providing it directly with the options parameter.");
  }
...
// node_modules/@dreamcomposer/pre/lib/pcm/material.js
var import_tinycolor2 = __toESM(require_tinycolor(), 1);
function isPcmMaterial(obj) {
  return isPcmItem(obj) && "previewColor" in obj && "definition" in obj;
}
function isPcmMaterialMinimalData(obj) {
  return typeof obj === "object" && obj !== null && "definition" in obj && "supportsOverprintContent" in obj && "supportsOverprintColor" in obj && "isVarnishMaterial" in obj && "isPrintingMaterial" in obj && "isSurfaceMaterial" in obj && "overprintMode" in obj;
}
async function getMaterials(accessToken = getAccessToken(), baseUrl = getPcmBaseUrl()) {
  const items = await _getPcmItems("materials", accessToken, baseUrl);
  return items.map(mapNativePcmMaterial);
}
async function getMaterialById(id, accessToken = getAccessToken(), baseUrl = getPcmBaseUrl()) {
  const item = await _getPcmItem("materials", id, accessToken, baseUrl);
  return item ? mapNativePcmMaterial(item) : void 0;
}
function mapNativePcmMaterial(material) {
...
function getMaterialBaseColor(material) {
  ...
  const c = (0, import_tinycolor2.default)(colorString).toRgb();
...
function getMaterialBaseColorString(material) {
  ...
  if (materialDefinition != null && typeof materialDefinition === "object" && "materialOverrides" in materialDefinition) {
    const materialConfigs = materialDefinition["materialOverrides"];
    if (!isArray_default(materialConfigs)) {
...
function getAlbedoColorFromConfigEntry(configEntry) {
  if (configEntry != null && typeof configEntry === "object" && "albedoColor" in configEntry) {
...
// node_modules/@dreamcomposer/pre/lib/pcm/pattern-embossing.js
function isPcmPatternEmbossing(obj) {
  return isPcmItem(obj) && "offset" in obj && "scaling" in obj && "overrides" in obj;
}
function isPcmPatternEmbossingDefinition(obj) {
  return typeof obj === "object" && obj !== null && "overrides" in obj;
}
async function getPatternEmbossings(accessToken = getAccessToken(), baseUrl = getPcmBaseUrl()) {
...
async function getPatternEmbossingById(id, accessToken = getAccessToken(), baseUrl = getPcmBaseUrl()) {
  const item = await _getPcmItem("pattern_embossings", id, accessToken, baseUrl);
  return item ? mapNativePcmPatternEmbossingToLibrary(item) : void 0;
}
var mapNativePcmPatternEmbossingToLibrary = (patternEmbossing) => ({
...
// node_modules/@dreamcomposer/pre/lib/methods/layers.js
var setLayers = ensureCallOrder(_setLayers);
async function _setLayers(dcController, layers) {
  const payload = {};
  await Promise.all(layers.map(async (layer, index) => {
    ...
    if (layer.overprintContent && !material.supportsOverprintContent) {
      throw new Error(`Material of layer ${index} does not support 'overprintContent'.${material.isPrintingMaterial ? " It is a printed material on which the texture is always drawn by default." : ""}`);
    }
    ...
    if (((_c = layer.patternEmbossing) === null || _c === void 0 ? void 0 : _c.preset) && !material.supportsPatternEmbossing) {
    ...
    payload[layerName] = {
      ...
      overprintMode: material.overprintMode === "none" ? void 0 : material.overprintMode,
...
async function getPcmMaterialWithDefinition(dcController, material) {
...
// node_modules/@dreamcomposer/pre/lib/methods/load.js
async function load(dcController) {
  return await dcController.load();
}

// node_modules/@dreamcomposer/pre/lib/methods/loading-screen.js
var loadingScreenRenderer = /* @__PURE__ */ new Map();
function setLoadingScreen(dcController, render) {
...
// node_modules/@dreamcomposer/pre/lib/methods/on.js
function on(dcController, eventName, callback) {
...
// node_modules/@dreamcomposer/pre/lib/methods/ppi.js
function setPpi(dcController, ppi) {
  return dcController.postMessage({
    channel: "set-ppi",
    payload: ppi
  });
...
// node_modules/@dreamcomposer/pre/lib/methods/render-quality.js
var RenderQuality;
(function(RenderQuality2) {
  RenderQuality2[RenderQuality2["VERY_LOW"] = 1.5] = "VERY_LOW";
  RenderQuality2[RenderQuality2["LOW"] = 1.25] = "LOW";
  RenderQuality2[RenderQuality2["MEDIUM"] = 1] = "MEDIUM";
  RenderQuality2[RenderQuality2["HIGH"] = 0.75] = "HIGH";
  RenderQuality2[RenderQuality2["VERY_HIGH"] = 0.5] = "VERY_HIGH";
})(RenderQuality || (RenderQuality = {}));
function setRenderQuality(dcController, quality) {
  return dcController.postMessage({
...
// node_modules/@dreamcomposer/pre/lib/methods/reset-camera-position.js
function resetCameraPosition(dcController) {
  return dcController.postMessage({
    channel: "reset-camera-position",
    payload: {}
  }, {
    memorize: false
  });
...
// node_modules/@dreamcomposer/pre/lib/methods/update-camera-position.js
function updateCameraPosition(dcController, changes, transitionDuration) {
  return dcController.postMessage({
    channel: "update-camera-position",
    payload: { changes, transitionDuration }
  }, {
    memorize: false
  });
}

// node_modules/@dreamcomposer/pre/lib/methods/geometry-transform.js
function updateGeometryTransform(dcController, changes, transitionDuration) {
  return dcController.postMessage({
...
function setInitialGeometryTransform(dcController, transform2) {
  return dcController.postMessage({
    channel: "set-initial-geometry-transform",
    payload: { transform: transform2 }
  }, {
    memorize: false
  });
}
function resetGeometryTransform(dcController, transitionDuration) {
  return dcController.postMessage({
...
function placeGeometryOnGround(dcController, transitionDuration) {
  return dcController.postMessage({
...
// node_modules/@dreamcomposer/pre/lib/methods/rotate-to-vector.js
function rotateToVector(dcController, vector) {
  return dcController.postMessage({
    channel: "rotate-to-vector",
    payload: vector
  }, {
    memorize: false
  });
...
// node_modules/@dreamcomposer/pre/lib/methods/set-preview.js
function isPreviewRenderOptions(options) {
  return options.render !== void 0;
}
function setPreview(dcController, options) {
  const previewRenderer = isPreviewRenderOptions(options) ? () => options.render(dcController.iFrameContainer) : () => renderStyledPreview(dcController, options);
  ...
  container.innerHTML = `
        <div class="dc-preview">
            <button>
                ...
                <div${placeholderImageSrc ? ` style="background-image: url('${placeholderImageSrc}');"` : ""}></div>
            </button>
        </div>
    `;
  container.querySelector("button").addEventListener("click", () => load(dcController));
}

// node_modules/@dreamcomposer/pre/lib/methods/set-use-framing-animation.js
function setUseFramingAnimation(dcController, enabled) {
  return dcController.postMessage({
    channel: "set-use-framing-animation",
    payload: enabled
  });
}

// node_modules/@dreamcomposer/pre/lib/methods/shadowplane-visibility.js
function setShadowPlaneVisibility(dcController, visible) {
  return dcController.postMessage({
    channel: "set-shadowplane-visible",
    payload: visible
  });
}

// node_modules/@dreamcomposer/pre/lib/methods/skeletons.js
async function setSkeletonStates(dcController, states) {
  const payload = {};
  states.map((state) => {
    var _a;
    ...
    (_a = state.boneStates) === null || _a === void 0 ? void 0 : _a.forEach((boneState) => {
      var _a2;
      (_a2 = payload[skeletonName].boneStates) === null || _a2 === void 0 ? void 0 : _a2.push({
...
// node_modules/@dreamcomposer/pre/lib/methods/stage-mappings.js
async function setStageMappings(dcController, mappings) {
  await dcController.postMessage({
    channel: "set-stage-mappings",
    payload: Object.fromEntries(Object.entries(mappings).map(([stageName, mapping]) => {
      var _a;
      return [
        stageName,
        {
          uvMapping: mapping.uvMapping,
          subMeshUVArea: mapping.subMeshUVArea,
          rotation: (_a = mapping.rotation) !== null && _a !== void 0 ? _a : 0,
...
// node_modules/@dreamcomposer/pre/lib/methods/surface-material.js
var setSurfaceMaterial = ensureCallOrder(_setSurfaceMaterial);
...
// node_modules/@dreamcomposer/pre/lib/methods/toggle-fullscreen.js
function toggleFullscreen(dcController) {
  if (document.fullscreenElement) {
    document.exitFullscreen();
  } else {
    dcController.container.requestFullscreen();
  }
  return Promise.resolve();
}

// node_modules/@dreamcomposer/pre/lib/methods/toggle-inspector.js
function toggleInspector(dcController) {
  if (!dcController.isLoaded()) {
    throw new Error("Inspector cannot be toggled when viewer is not loaded.");
  }
  return dcController.postMessage({
    channel: "toggle-inspector",
    payload: {}
  }, {
    memorize: false
  });
...
// node_modules/@dreamcomposer/pre/lib/methods/unload.js
function unload(dcController) {
  return dcController.unload();
}

// node_modules/@dreamcomposer/pre/lib/methods/update-environment-rotation.js
function updateEnvironmentRotation(dcController, rotation, transitionDuration) {
  return dcController.postMessage({
    channel: "update-environment-rotation",
    payload: { rotation, transitionDuration }
  }, {
    memorize: false
  });
}

// node_modules/@dreamcomposer/pre/lib/methods/annotationTags.js
var unmountFunctions = /* @__PURE__ */ new Map();
async function setAnnotationTags(dcController, tags, renderLabel) {
...
function createAnnotationTagContainers(dcController, tags, renderTag) {
  ...
  tags.forEach((l) => {
    ...
    let div = tagDivs.find((div2) => div2.id === divId);
    if (div == null) {
...
function updateAnnotationTagPositions(dcController, tags) {
  ...
  tags.forEach((t, idx) => {
    ...
    const y = t.tagAnchorPosition.screenPosition.y * containerRect.height - rect.height / 2;
    if (t.tagAnchorPosition.screenPosition.x < t.objectAnchorPosition.screenPosition.x) {
      x -= rect.width;
    }
...
// node_modules/@dreamcomposer/pre/lib/methods/custom-request-headers.js
function setCustomHeaders(dcController, headers) {
  return dcController.postMessage({
    channel: "set-custom-request-headers",
    payload: headers
  });
}

// node_modules/@dreamcomposer/pre/lib/pcm/index.js
var pcm_exports = {};
__export(pcm_exports, {
  createGeometry: () => createGeometry,
  deleteGeometry: () => deleteGeometry,
  disablePcmAccessTokenCheck: () => disablePcmAccessTokenCheck,
  getEmbossingConfigurationById: () => getEmbossingConfigurationById,
  getEmbossingConfigurations: () => getEmbossingConfigurations,
  getEnvironmentById: () => getEnvironmentById,
  getEnvironments: () => getEnvironments,
  getFeatures: () => getFeatures,
  getGeometries: () => getGeometries,
  getGeometryById: () => getGeometryById,
  getMaterialBaseColor: () => getMaterialBaseColor,
  getMaterialById: () => getMaterialById,
  getMaterials: () => getMaterials,
  getPatternEmbossingById: () => getPatternEmbossingById,
  getPatternEmbossings: () => getPatternEmbossings,
  isPcmEmbossingConfiguration: () => isPcmEmbossingConfiguration,
  isPcmEnvironment: () => isPcmEnvironment,
  isPcmFeature: () => isPcmFeature,
  isPcmGeometry: () => isPcmGeometry,
  isPcmMaterial: () => isPcmMaterial,
  isPcmMaterialMinimalData: () => isPcmMaterialMinimalData,
  isPcmPatternEmbossing: () => isPcmPatternEmbossing,
  isPcmPatternEmbossingDefinition: () => isPcmPatternEmbossingDefinition,
  mapNativePcmMaterial: () => mapNativePcmMaterial,
  patchGeometry: () => patchGeometry
});

// node_modules/@dreamcomposer/pre/lib/pcm/feature.js
function isPcmFeature(obj) {
  return isPcmItem(obj) && "flag" in obj;
}
async function getFeatures(accessToken = getAccessToken(), baseUrl = getPcmBaseUrl()) {
  const items = await _getPcmItems("features", accessToken, baseUrl);
  return items.map(mapNativePcmFeatureToLibrary);
}
var mapNativePcmFeatureToLibrary = (feature) => ({
  ...mapNativePcmItemToPcmItem(feature),
  flag: feature.flag
});

// node_modules/@dreamcomposer/pre/lib/pcm/embossing.js
function isPcmEmbossingConfiguration(obj) {
  return isPcmItem(obj) && "iconUrl" in obj && "isCustomByBrightness" in obj && "level" in obj && "size" in obj && "sizeCutoff" in obj && "curve" in obj;
}
async function getEmbossingConfigurations(accessToken = getAccessToken(), baseUrl = getPcmBaseUrl()) {
...
async function getEmbossingConfigurationById(id, accessToken = getAccessToken(), baseUrl = getPcmBaseUrl()) {
  const item = await _getPcmItem("embossings", id, accessToken, baseUrl);
  return item ? mapNativePcmEmbossingConfigurationToLibrary(item) : void 0;
}
var mapNativePcmEmbossingConfigurationToLibrary = (embossingConfig) => ({
...
export {
  ...
  setCutoutTexture,
  setEffectSettings,
  setEmbossingHeightTexture,
  setEnvironment,
  setEnvironmentVisibility,
  setGeometry,
  setGridVisibility,
  setInitialGeometryTransform,
  setLayers,
  setLoadingScreen,
  setPpi,
  setPreview,
  setRenderQuality,
  setShadowPlaneVisibility,
  setSkeletonStates,
  setStageMappings,
  setSurfaceMaterial,
  setUseFramingAnimation,
  toggleFullscreen,
  toggleInspector,
  unload,
...
lodash-es/lodash.js:
   ...
   *)
*/
//# sourceMappingURL=@dreamcomposer_pre.js.map
